...
public class EightQueensFullSolution {
        ...
        while (currentRow >= 0) {
            if (isValid(currentRow, currentColumn, board)) {
                ...
                if (currentRow >= board.length) {
                   ...
                   // printBoard(board);
                    currentRow--;
                    currentColumn = board[currentRow] + 1; // brilliant: just move the existing queen 1 pos to the right,
                    // then we search on, as if we never had found a solution !
                }
            }
            else {
